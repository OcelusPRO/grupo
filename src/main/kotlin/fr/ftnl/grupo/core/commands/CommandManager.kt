...
class CommandManager {
	...
	private inline fun <reified T: ICmd, U: GenericInteractionCreateEvent> execCmd(e: U, filter: (U) -> String, action: (T) -> Unit) {
		val cmd = ICmd.cmd.filterIsInstance<T>().find { it.name.startsWith(filter.invoke(e)) } ?: return
		
		if (!cmd.allowDM && !e.isFromGuild)
			return reply(e, MessageCreateBuilder()
							 .setContent("Cette commande n'est pas disponible en messages privées"
								.toLang(e.userLocale, LangKey.keyBuilder(this, "handle.interaction", "not_available_in_dm")))
							 .build())
		...
		val cooldownMessage =
			MessageCreateBuilder()
				.setContent(":timer: Vous devez encore attendre %ss avant de pouvoir utiliser cette commande."
					.toLang(e.userLocale, LangKey.keyBuilder(this, "handle.interaction", "cooldownMessage"))
	...
	suspend fun dispatch(event: GenericInteractionCreateEvent) {
		when (event) {
			is SlashCommandInteractionEvent ->
				execCmd<ISlashCmd, SlashCommandInteractionEvent>(event, {it.name}) { it.action(event) }
			is MessageContextInteractionEvent ->
				execCmd<IMessageCmd, MessageContextInteractionEvent>(event, {it.name}) { it.action(event) }
			is UserContextInteractionEvent ->
				execCmd<IUserCmd, UserContextInteractionEvent>(event, {it.name}) { it.action(event) }
			is CommandAutoCompleteInteractionEvent ->
				execCmd<ISlashCmd, CommandAutoCompleteInteractionEvent>(event, {it.name}) { it.action(event) }
			is ButtonInteractionEvent ->
				execCmd<IButtonCmd, ButtonInteractionEvent>(event, {it.componentId}) { it.action(event) }
			is StringSelectInteractionEvent ->
				execCmd<ISelectCmd, StringSelectInteractionEvent>(event, {it.componentId}) { it.action(event) }
			is ModalInteractionEvent ->
				execCmd<IModalCmd, ModalInteractionEvent>(event, {it.id}) { it.action(event) }
		}
	}
}